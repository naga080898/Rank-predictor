...
def extract_candidate_info(doc):
...
def parse_response_sheet(pdf_path: str, positive_marks: float = 1.0, negative_marks: float = 0.25, output_dir: str = "output_parsed"):
    """
    Parses candidate response sheet PDF:
    - Extracts all questions (Q.1, Q.2 ... Q.100)
    ...
    logger.info(f"Opening response sheet: {pdf_file.name}")
    doc = pymupdf.open(pdf_file)
    candidate_info = extract_candidate_info(doc)
    logger.info(f"Candidate: '{candidate_info['participant_name']}' | Hall Ticket: '{candidate_info['hall_ticket']}' | Subject: '{candidate_info['subject']}'")
    ...
    # Step 2: Locate Question Anchors (Q.1, Q.2, ... Q.100)
    q_start_indices = []
    seen_q_nums = set()
    for idx, span in enumerate(all_pages_spans):
        # Match 'Q.1', 'Q.2', 'Q.100' or 'Q.53 For the following circuit'
        m = re.match(r'^Q\.?\s*(\d+)(?:\s|\.|$)', span["text"])
        if m:
            q_num = int(m.group(1))
            # Avoid duplicate anchors within consecutive spans
            if q_num not in seen_q_nums:
                q_start_indices.append((idx, q_num))
                seen_q_nums.add(q_num)

    # Sort anchors by question number or appearance
    logger.info(f"Identified {len(q_start_indices)} questions.")
    ...
    # Calculate overall summary & sections
    ...
    out_file = out_path / f"{pdf_file.stem}_evaluated.json"
    with open(out_file, "w", encoding="utf-8") as f:
        json.dump(result, f, indent=2, ensure_ascii=False)

    logger.info(f"Summary for {candidate_info['participant_name'] or pdf_file.name}: Score={final_score}, Correct={correct_cnt}, Incorrect={incorrect_cnt}, Unattempted={unattempted_cnt}")
    return result

if __name__ == "__main__":
    target = sys.argv[1] if len(sys.argv) > 1 else "sample_pdfs/aee response sheet  12.28.26 PM.pdf"
    parse_response_sheet(target)
